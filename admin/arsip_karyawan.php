...
<div class="container">
    ...
    if (isset($_GET['pesan'])) {
        if ($_GET['pesan'] == "oke") {
            echo "<div class='alert alert-success'>Karyawan berhasil diaktifkan kembali!</div>";
        }
    }
    ?>
    <?php
    if (isset($_GET['pesan'])) {
        if ($_GET['pesan'] == "data_tidak_ditemukan") {
            echo "<div class='alert alert-danger'>Data arsip gagal ditemukan!</div>";
        }
    }
    ?>
    <?php
    if (isset($_GET['pesan'])) {
        if ($_GET['pesan'] == "gagal_menghapus") {
            echo "<div class='alert alert-danger'>Arsip karyawan gagal dihapus!</div>";
        }
    }
    ?>
    <?php
    if (isset($_GET['pesan'])) {
        if ($_GET['pesan'] == "gagal_insert") {
            echo "<div class='alert alert-danger'>Arsip karyawan gagal diaktifkan!</div>";
        }
    }
    ?>
    <?php
    if (isset($_GET['pesan'])) {
        if ($_GET['pesan'] == "terhapus") {
            echo "<div class='alert alert-danger'>Karyawan berhasil dihapus!</div>";
        }
    }
    ?>
    <div class="panel" data-aos="fade-down" data-aos-delay="200">
        <div class="panel-heading">
            <h4>Arsip Karyawan</h4>
        </div>
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped">
                ...
                #$data = mysqli_query($koneksi, "select * from arsip_karyawan");
                $no = 1;
        ...
        status LIKE '%$search%'");
                    ...
                    $query = mysqli_query($koneksi, "SELECT * FROM arsip_karyawan");
                }
                ...
                ?>
                    ...
                    ?> 
                        ...
                        <td>
                            <a href="unarsip_karyawan.php?id=<?php echo $d['nik']; ?>" class="btn btn-sm btn-success">Batal</a>
                            <a href="arsip_hapus.php?id=<?php echo $d['nik']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus <?= $d['nik'] ?>?')">Hapus</a>
                        </td>
                    </tr>
                <?php
                }
