...
if ($data) {
    ...
    $insert = mysqli_query($koneksi, "INSERT INTO arsip_karyawan 
    ...
    if ($insert) {
        ...
        $aksi = "Mengarsipkan data karyawan dengan NIK $nik.";
        mysqli_query($koneksi, "INSERT INTO audit_log (username, aksi) VALUES ('$username', '$aksi')");

        if ($delete) {
            header("location:karyawan.php?pesan=diarsipkan");
        } else {
            header("location:karyawan.php?pesan=gagal_menghapus");
        }
    } else {
        header("location:karyawan.php?pesan=gagal_insert_arsip");
    }
} else {
